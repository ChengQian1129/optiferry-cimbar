...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        frameAnalyzer = QrFrameAnalyzer(
            onDecodedBatch = { decodedBatch ->
                ...
                if (highSpeedBytes.isNotEmpty() && pendingProtocolFrames.get() < 8) {
        ...
        protocolExecutor.execute {
            val saved = batchStore.incompleteTransfers()
            ContextCompat.getMainExecutor(this).execute {
                if (!cameraStarted && saved.isNotEmpty()) {
                    fileText.text = "${saved.size} 个未完成传输 · 开始扫描后自动恢复"
                    missingText.text = saved.take(3).joinToString("\n") { "${it.name}: ${it.done}/${it.count} segments" }
                }
            }
        }
        fileText.setOnLongClickListener {
            protocolExecutor.execute {
                val saved = batchStore.incompleteTransfers()
                ContextCompat.getMainExecutor(this).execute {
                    androidx.appcompat.app.AlertDialog.Builder(this)
                        .setTitle("未完成传输")
                        .setMessage(saved.joinToString("\n") { "${it.name}: ${it.done}/${it.count} segments" }.ifEmpty { "无未完成传输" })
    ...
    private fun maybeSoftDecoderRecover(stats: ScanStats) {
        if (batchStopped || !cameraStarted || processRestarting) return
    ...
    private fun maybeRephaseDualCamera(stats: ScanStats) {
        if (batchStopped || !cameraStarted || bindingCamera || processRestarting || requestedFps != 60) return
        if (inHalRecoveryWarmup()) return
    ...
    private fun handleHighSpeedFrame(bytes: ByteArray) {
        ...
        if (batchStore.completedFrame(bytes)) { highLastFrameAt = SystemClock.elapsedRealtime(); return }
        val now = SystemClock.elapsedRealtime()
        highFrameCount += 1
        ...
        if (file != null && Bfb1.isBatch(file.bytes)) {
            try {
                val segment = Bfb1.parse(file.bytes)
